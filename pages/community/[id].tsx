...
interface PostWithUserAndAnswer extends Post {
  user: User;
...
const CommunityPostDetail: NextPage = () => {
  ...
  const { data, mutate } = useSWR<CommunityPostResponse>(router.query.id ? `/posts/${router.query.id}` : null);
  const { register, handleSubmit, reset } = useForm<AnswerForm>();
  const [ wonder, { loading, } ] = useMutation<AnswerResponse>(`/api/posts/${router.query.id}/wonder`);
  const [ sendAnswer, {data:answerData, loading:answerLoading, } ] = useMutation(`/api/posts/${router.query.id}/answer`);
    ...
    mutate(
      {
        ...
        post: {
          ...
          _count: {
            ...data?.post._count, 
            wonderings: data?.post._count.wonderings + (data.isWondering ? -1 : 1)
          }
        },
        ...
        {data?.post?.answers?.map(answer => (
          <div key={answer.id} className="flex items-start space-x-3">
            ...
            <div>
              ...
              <span className="text-xs text-gray-500 block ">{answer.user.createdAt}</span>
